---
layout: post
title:      "Building a Simple but Effective Regression Model:"
date:       2020-07-19 23:54:58 +0000
permalink:  building_a_simple_but_effective_regression_model
---


For my second project, I explored housing prices in King County Washington and built a model to accurately predict other home prices in the area.  With this model, we can also explore different variables and tell which ones are likely to have the largest impact on price.<br>
...
The data for this model was already fairly clean.  It holds 21 variables, and only a few needed some minor cleaning. I completed all the cleaning using Pandas. The columns provided are listed below:<br><br>
...
Being a long time King County Resident, this task was on the easier side for me.  I collected a list of zip codes from the King County website, and used google maps and my own knowledge to group together different cities.  For example, there were several zip codes up north that I grouped together as ‘North KC’, while I split Seattle itself into several parts.  Given the variety of home prices in Seattle as a whole, and even the fact that the geography splits it up so much, this made sense.

## Impact of Geography on Price:

Next, I used a mapping software called Folium to tell which areas, not just zip codes, have the most expensive homes. Here are the results:
...
I also looked at the impact of grade on home price.  Here is some information on the grading system from the King County Assessment Site:
...
<br>
This box plot shows that there is a tremendous difference between different grades.<br>
![Imgur](https://i.imgur.com/odKvlxL.png)

As you can see, grade is quite important when it comes to home price and was crucial in my model later on.  Buying, renovating, and flipping can be highly profitable! Especially if you are taking a 5 rated home (just above average) and upgrading it to a 6 by upgrading the interior and exterior finishing touches. There is a 200K difference between the two medians! It seems very doable to turn a 5 into a 6 with less than 200K, enabling you to capture a profit.

# Regression Modeling: 

## Baseline Model:
After some feature engineering and other cleaning steps (also using Pandas), I created this baseline model.  It's important to have a simple model with few transformations or polynomial features in order to have a baseline to compare other models against.  The feature engineering consisted mostly of changing categorical variables into dummy variables.  This made it easier for the model to understand.  Here is the result of the first model:

![Imgur](https://i.imgur.com/LTjTwGl.png)

This was a great start!  Root Mean Squared Error was fairly large at around $150K, but R squared was quite high at over .9.  Additionally, there's only a $700 dollar difference between the root mean squared error of the train and test data! This means that so far, we don't have to worry about over-fitting. Right now, our main concern is keeping the R squared high while reducing RMSE. 
<br>With that done, I was ready to test the following:
